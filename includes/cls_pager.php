...
class page_1 extends page
{
	...
	function page_all_num($table,$where)
	{
		...
		$sql = "SELECT * FROM ".$ea->table($table). $where;
	...
	*/
	...
	function page_array($param="*",$table,$where,$order,$curr_page,$page_num)
	{
		...
		//开始查询数据
		$sql = 'SELECT '.$param.' FROM '.$ea->table($table). $sql_where . $sql_order . ' LIMIT '.$curr_page .' , '.$page_num;
...
class page_2{
	...
	function getNext($table,$uid,$id){
			...
			$sql = 'SELECT '.$uid.', dateline, orderby FROM '.$ea->table($table).' WHERE '.$uid.' = '.$id.' limit 0,1';
			...
			$sql = 'SELECT '.$uid.' FROM '.$ea->table($table).' WHERE orderby >= '.$curorder.' and dateline <= '.$curtime.' ';
