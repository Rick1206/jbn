...
function add_news($param,$admin_id)
{
	...
	$sql = 'INSERT INTO '.$ea->table('news'). '(`title`,`keywords`,`contant`,`lan`,`add_time`,`is_publish`,`order`,`admin_id`) '.
			'VALUES ("'.$param['title'].'",'.
			'"'.$param['keywords'].'",'.
			'"'.$param['contant'].'",'.
			//'"'.$param['summary'].'",'.
	...
	{	//add global search
		...
		{
			//del add news 
			$del_add = 'DELETE FROM '.$ea->table('news').' WHERE news_id='.$id;
...
function search_infor($id)
{
	...
	$sql = 'SELECT * FROM '.$ea->table('news'). 'WHERE news_id='.$id;
	$res = $db->getAll($sql);
...
function del_news($id)
{
	...
	else
	{
		...
		$sql = 'DELETE FROM '.$ea->table('news').' WHERE news_id='.$id;
