...
function create_news_group($param,$right,$admin_id)
{
  ...
  $res = $db->query('SELECT name FROM '.$ea->table('user_group').'where name="'.$param['right_name'].'"');
  $num = $db->num_rows($res);
  if($num>=1)
  {
  	return 0;
  ...
  else
  {
	  ...
	  $sql='INSERT INTO '.$ea->table('user_group').' (`name`,`right`,`desc`,`admin_id`) VALUES '.
			'('.'"'.$param['right_name'].'",'.
			'"'.$right.'",'.
			'"'.$param['desc'].'",'.
			'"'.$admin_id.'"'.
...
function del_user_group($id)
{
	...
	else
	{
		...
		$res = $db->query('SELECT user_group FROM '.$ea->table('admin').' WHERE user_group='.$id);
		...
		else
		{
			...
			$sql = 'DELETE FROM '.$ea->table('user_group').' WHERE j_id='.$id;
			$res = $db->query($sql);
			if($res)
			{
				return true;
...
{	
	...
	else
	{
	    if($param['right_name1']!= $param['right_name2'])
		{
			...
			$res = $db->query('SELECT name FROM '.$ea->table('user_group'). ' WHERE name="'.$param['right_name1'].'"');
			$num = $db->num_rows($res);
			//echo $num;
			if( $num >= 1 )
			{
			...
			{
				$sql = ' UPDATE	'.$ea->table('user_group').
						' SET'.
...
function get_user_group($id)
{
	...
	if( empty($id) )
	{
		$sql = 'SELECT * FROM '.$ea->table('user_group');
		$res = $db->getAll($sql);
		return $res;
	}
	else
	{
		$sql = ' SELECT * FROM '.$ea->table('user_group') . ' WHERE j_id='.$id;
		$res = $db->getAll($sql);
		$nam = $res[0]['name'];
		return $nam;
	}
...
{
	global $db,$ea;
	$sql = ' SELECT * FROM '.$ea->table('user_group') . ' WHERE j_id='.$id. ' LIMIT 1';
...
function get_right_byId($id)
{
	...
	else
	{
		...
		$sql = 'SELECT admin_id,user_group FROM '.$ea->table('admin'). 'WHERE admin_id='.$id;
		...
		else
		{
			...
			$sql = 'SELECT `j_id`,`right` FROM '.$ea->table('user_group'). ' WHERE j_id='.$group_id;
			$res = $db->getAll($sql);
			$r_l = $res[0]['right'];
			$r_l = explode(',',$r_l);
			if(count($r_l)!=0)
...
function  get_right_details($id)
{
	...
	if(!empty($id))
	{
		...
		$sql = 'SELECT `j_id`,`right`,`name`,`desc` FROM '.$ea->table('user_group'). ' WHERE j_id='.$id;
		$res = $db->getAll($sql);
		return $res;
	}
	else
