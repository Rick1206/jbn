...
function attention_xml()
{
	global $ea,$db,$pic_doc;
		...
		/*导出XML文件开始*/
		/*获取数据开始*/
		$query_xml = $db->query("SELECT * FROM ".$ea->table('attention')." ORDER BY orderby, attention_id");
...
function team_xml()
{
	global $ea,$db,$pic_doc;
		...
		/*导出XML文件开始*/
		/*获取数据开始*/
		$query_xml = $db->query("SELECT * FROM ".$ea->table('team')." ORDER BY orderby, team_id");
		while ($this_xml = $db->fetch_array($query_xml)) {
			if ($this_xml['name_en']) {
			$maindata .= "\t\t<list>\r\n".
						 "\t\t\t<photo>".$pic_dir.$this_xml['photo']."</photo>\r\n".
...
function gallery_xml()
{
	global $ea,$db,$pic_doc;
		...
		/*导出XML文件开始*/
		/*获取数据开始*/
		$query_xml = $db->query("SELECT * FROM ".$ea->table('gallery')." ORDER BY orderby, gallery_id");
		while ($this_xml = $db->fetch_array($query_xml)) {
			if ($this_xml['name_en']) {
			$maindata .= "\t\t<list>\r\n".
