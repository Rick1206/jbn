       ...
       <tr>
        ...
        <tr>
          ...
          <?php
            ...
            $oFCKeditor->BasePath	= $sBasePath ;
            $oFCKeditor->Value		= $news_description_cn;
            $oFCKeditor->ToolbarSet		= 'Basic' ;
            $oFCKeditor->Config['AutoDetectLanguage']		= true ;
            $oFCKeditor->Config['DefaultLanguage']		= 'en' ;
            $oFCKeditor->Height		= 150 ;
	...
	elseif($action == 'modify_news')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('news')." SET photo='".$pic."' WHERE news_id='$get_news_id'");
				echo "";
		  }
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('news')." SET image='".$pic2."' WHERE news_id='$get_news_id'");
		  }

		  $back_data = $db->query("UPDATE ".$ea->table('news')." SET title_en='".$param['title_en']."', intro_cn='".$param['intro_cn']."', title_cn='".$param['title_cn']."', intro_en='".$param['intro_en']."', description_en='".htmlspecialchars($param['description_en'])."', description_cn='".htmlspecialchars($param['description_cn'])."', dateline='".$param['add_time']."', orderby='".$param['orderby']."' WHERE news_id='$get_news_id'");
