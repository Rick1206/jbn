...
<?php
   ...
   if(empty($action))
   {
	if($get_finance_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('finance'). 'WHERE finance_id='.$get_finance_id;
...
<?php
	...
	elseif($action == 'modify_finance')
	{
		  ...
		  $back_data = $db->query("UPDATE ".$ea->table('finance')." SET title_cn='".$param['pro_cn']."', title_en='".$param['pro_en']."', ftype='".$param['ftype']."', fmoney='".$param['count']."' WHERE finance_id='$get_finance_id'");
