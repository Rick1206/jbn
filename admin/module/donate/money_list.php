...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <form id="form2" name="form2" method="get" action="money_list.php">
  <tr>
    <td width="5" height="31" background="../../images/i_m_t.jpg">&nbsp;</td>
    <td width="486" background="../../images/i_m_t.jpg">爱心捐献&gt;&gt;财力支持</td>
  ...
  <tr>
    ...
    <td height="400" colspan="2" valign="top">
	  <form id="form1" name="form1" method="post" action="money_list.php" >
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
        <tr>
          <td width="7" bgcolor="#999999" >&nbsp;</td>
          <td colspan="6" class="m_t_c">财力支持</td>
		...
		if (isset($_GET['formdate']) || isset($_GET['todate'])) {
			...
			$where = empty($money_type) ? "" : " WHERE money_type = '".$money_type."'";
			//to list money information
            $all_date_num = page_1::page_all_num('money',$conditions);
            //$last_page = intval (($all_date_num - 1) / $page_a) + 1; //获得总页数
            $page = isset($_GET['page']) ? $_GET['page'] : 1 ;	
            $offset = ($page - 1) * $page_a;
            $user_list = page_1::page_array('*','money',$conditions,' ORDER BY dateline DESC, money_id DESC',$offset,$page_a);
        ...
        <tr>
          <td>&nbsp;</td>
          <td height="25">&nbsp;</td>
          <td><input type="checkbox" name="del_money_a[]" value="<?php echo $v['money_id']; ?>" /></td>
          <td><?php echo $v['uname'];?></td>
          <td><?php echo $v['ucard'];?>：<?php echo $v['ucardinfo'];?></td>
          <td><?php echo $v['uphone'];?></td>
          <td><?php echo $v['umoney'];?></td>
          <td><?php echo $v['dateline'];?></td>
          <td><?php echo $_money_invoice[$v['uinvoice']]; ?></td>
        </tr>
        <?php
            }
        ?>
        <tr>
          ...
          <?php 
            echo page::page_num($all_date_num,$page_a,$page,'money_list.php');
          ?>
          </td>
        </tr>
        <tr>
          ...
          <td height="30" colspan="11" align="center">
            <input type="submit" name="del_money" id="del_money" value="删除" />   
...
<?php
	...
	else
	{
		...
		if(isset($param['del_money_a']))
		{
			
			if( isset($param['del_money_a']) && is_array($param['del_money_a']))
			{
				//删除数据
				foreach($param['del_money_a'] as $k)
				{
					$back_data = $db->query("DELETE FROM ".$ea->table('money')." WHERE money_id = '$k'");
				}
				
				//返回结果
				if($back_data)
				{
					show_msg('删除成功','money_list.php');
				}
				else
				{
					show_msg('请重试','money_list.php');
				}
		...
		{
			show_msg('没有选中删除项','money_list.php');
		}
