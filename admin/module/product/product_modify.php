...
<?php
   ...
   if(empty($action))
   {
	if($get_product_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('product'). 'WHERE product_id='.$get_product_id;
	...
	elseif($action == 'modify_product')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('product')." SET photo='".$pic."' WHERE product_id='$get_product_id'");
		  }
		  ...
		  $back_data = $db->query("UPDATE ".$ea->table('product')." SET title_cn='".$param['pro_cn']."', title_en='".$param['pro_en']."', pcount='".$param['count']."', orderby='".$param['orderby']."' WHERE product_id='$get_product_id'");
