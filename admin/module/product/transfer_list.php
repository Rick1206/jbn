    ...
    <td height="400" colspan="2" valign="top">
      ...
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
        ...
        <?php
			...
			$query = $db->query("SELECT * FROM ".$ea->table('transfer')." $where ORDER BY dateline Desc LIMIT $offset, $page_a");
	...
	else
	{
		...
		if(isset($param['del_transfer_a']))
		{
			...
			if( isset($param['del_transfer_a']) && is_array($param['del_transfer_a']))
			{
				...
				{
					$back_data = $db->query("DELETE FROM ".$ea->table('transfer')." WHERE transfer_id = '$k'");
				}
