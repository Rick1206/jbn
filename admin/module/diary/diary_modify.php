...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td>
      ...
      <tr>
          <td height="20" align="right">孩子类别：</td>
          <td height="30">
            <select name="child_id">
              <?php
				...
				?>
				<option <?php echo $thisB['child_id']==$diary_child_id ? " selected=\"selected\"" : "";?> value='<?php echo $thisB['child_id']; ?>' /><?php echo $thisB['name_cn']; ?></option>
				<?php
				}
             	?>
            </select>
	...
	elseif($action == 'modify_diary')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('diary')." SET photo='".$pic."' WHERE diary_id='$get_diary_id'");
				echo "";
		  }
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('diary')." SET image='".$pic2."' WHERE diary_id='$get_diary_id'");
		  }

		  $back_data = $db->query("UPDATE ".$ea->table('diary')." SET title_en='".$param['title_en']."', intro_cn='".$param['intro_cn']."', title_cn='".$param['title_cn']."', intro_en='".$param['intro_en']."', description_en='".htmlspecialchars($param['description_en'])."', description_cn='".htmlspecialchars($param['description_cn'])."', dateline='".$param['add_time']."' WHERE diary_id='$get_diary_id'");
