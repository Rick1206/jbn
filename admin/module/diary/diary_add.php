            ...
            <select name="child_id">
              <?php
				...
				while($thisB = $db->fetch_array($query)){
				 	echo "<option value='".$thisB['child_id']."'/>".$thisB['name_cn']."</option>";
				}
              ?>
            </select>
          </td>
...
<?php
	...
	elseif($action == 'add_diary')
	{
		...
		$back_data = $db->query("INSERT INTO ".$ea->table('diary')." (child_id, title_en, intro_en, title_cn, intro_cn, description_en, description_cn, dateline, photo, image) VALUES ('".
								$param['child_id']."','".$param['title_en']."','".$param['intro_en']."', '".$param['title_cn']."', '".$param['intro_cn']."','".htmlspecialchars($param['description_en'])."', '".htmlspecialchars($param['description_cn'])."', '".$param['add_time']."','".$pic."','".$pic2."')");
