    ...
    <td height="400" colspan="2" valign="top">
      ...
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
        ...
        <tr>
          ...
          <td width="52">全选</td>
		  <td width="138">孩子姓名</td>
		  <td width="500">标题</td>
          <td width="117">日期</td>
          <td width="94">操作</td>
        </tr>
        <?php
			$join = " JOIN ".$ea->table('children')." on ".$ea->table('diary').".child_id = ".$ea->table('children').".child_id";
            $all_date_num = page_1::page_all_num('diary',$join);
            $page = isset($_GET['page']) ? $_GET['page'] : 1 ;	
            $offset = ($page - 1) * $page_a;
            $param = "name_cn,diary_id, title_cn,title_en,jbn_diary.dateline";
            $user_list = page_1::page_array($param,'diary',$join,' ORDER BY jbn_diary.dateline DESC, jbn_diary.diary_id DESC',$offset,$page_a);
	...
	else
	{
		...
		if(isset($param['del_diary_a']))
		{
			...
			if( isset($param['del_diary_a']) && is_array($param['del_diary_a']))
			{
				...
				{
					$back_data = $db->query("DELETE FROM ".$ea->table('diary')." WHERE diary_id = '$k'");
				}
