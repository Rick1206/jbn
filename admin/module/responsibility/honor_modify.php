...
<?php
   ...
   if(empty($action))
   {
	if($get_honor_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('honor'). 'WHERE honor_id='.$get_honor_id;
...
<?php
	...
	elseif($action == 'modify_honor')
	{
	  if(!empty($get_honor_id) )
	  {		  		  
		  $back_data = $db->query("UPDATE ".$ea->table('honor')." SET year='".$param['year']."', title_en='".$param['title_en']."', title_cn='".$param['title_cn']."', title_gb='".$param['title_gb']."', detail_en='".$param['detail_en']."', detail_cn='".$param['detail_cn']."', detail_gb='".$param['detail_gb']."', orderby='".$param['orderby']."' WHERE honor_id='$get_honor_id'");
