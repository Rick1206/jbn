...
<?php
	...
	elseif($action == 'add_foundation')
	{
		...
		if($attachment_s){
			...
			$back_data = $db->query("INSERT INTO ".$ea->table('foundation')." (title_en, title_cn, title_gb, photo, description_en, description_cn, description_gb, orderby) VALUES ('".$param['title_en']."', '".$param['title_cn']."', '".$param['title_gb']."', '".$pic."', '".htmlspecialchars($param['description_en'])."', '".htmlspecialchars($param['description_cn'])."', '".htmlspecialchars($param['description_gb'])."', '".$param['orderby']."')");
			if($back_data)
			{
