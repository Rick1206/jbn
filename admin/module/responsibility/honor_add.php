...
<?php
	...
	elseif($action == 'add_honor')
	{
			...
			$back_data = $db->query("INSERT INTO ".$ea->table('honor')." (year, title_en, title_cn, title_gb, detail_en, detail_cn, detail_gb, orderby) VALUES ('".$param['year']."', '".$param['title_en']."', '".$param['title_cn']."', '".$param['title_gb']."', '".$param['detail_en']."', '".$param['detail_cn']."', '".$param['detail_gb']."', '".$param['orderby']."')");
			if($back_data)
			{
				//honor_xml();
