...
<body>
<?php
   // to add community information
   $action = isset($param["action"]) ? $param["action"] : "";
   if(empty($action))
   {
?>
<form method="post" enctype="multipart/form-data" name="add_community" id="add_community">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="5" height="31" background="../../images/i_m_t.jpg">&nbsp;</td>
    <td width="972" background="../../images/i_m_t.jpg">
    企业公民&gt;&gt;<a href="community_list.php" id="module_link">我们在行动列表</a>&gt;&gt;添加我们在行动</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>
      ...
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td height="20">&nbsp;</td>
          <td height="30">
            <input type="submit" name="submit" id="add_community" value="保存" />
            <input type="hidden" name="action" value="add_community" />            </td>
        </tr>
      </table>
    </td>
  </tr>
...
<?php
	...
	elseif($action == 'add_community')
	{
		...
		if($attachment_s){
			...
			$back_data = $db->query("INSERT INTO ".$ea->table('community')." (title_en, title_cn, title_gb, photo, description_en, description_cn, description_gb, orderby) VALUES ('".$param['title_en']."', '".$param['title_cn']."', '".$param['title_gb']."', '".$pic."', '".htmlspecialchars($param['description_en'])."', '".htmlspecialchars($param['description_cn'])."', '".htmlspecialchars($param['description_gb'])."', '".$param['orderby']."')");
			if($back_data)
			{
				show_msg('添加成功','community_add.php');  
			}
			else
			{
				show_msg('请重试','community_add.php');
			}
		} else {
			show_msg('图片上传失败','community_add.php');
		}
