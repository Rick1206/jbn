...
<?php
   ...
   if(empty($action))
   {
	if($get_banner_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('banners'). 'WHERE banner_id='.$get_banner_id;
	...
	elseif($action == 'modify_banner')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('banners')." SET photo='".$pic."' WHERE banner_id='$get_banner_id'");
		  }

		  /*if (isset($param['image_del']) && $param['image_del']=="1") {
		  	@unlink($attachdir.$param['event_image']);
			$db->query("UPDATE ".$ea->table('event')." SET image='' WHERE event_id='$get_banner_id'");
		  }*/
		  ...
		  /*if($attachment == 1)
		  {
				...
				@unlink($attachdir.$param['event_image']);
				$db->query("UPDATE ".$ea->table('banners')." SET image='".$pic2."' WHERE banner_id='$get_banner_id'");
		  }*/

		  $back_data = $db->query("UPDATE ".$ea->table('banners')." SET lan='".$param['lan_type']."', link='".$param['link']."', orderby='".$param['orderby']."' WHERE banner_id='$get_banner_id'");
