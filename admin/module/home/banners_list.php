    ...
    <td height="400" colspan="2" valign="top">
      ...
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
        ...
        <?php
			...
			$query = $db->query("SELECT * FROM ".$ea->table('banners')." $where ORDER BY lan, orderby LIMIT $offset, $page_a");
	...
	else
	{
		...
		if(isset($param['del_banner_a']))
		{
			...
			if( isset($param['del_banner_a']) && is_array($param['del_banner_a']))
			{
				...
				{
					$back_data = $db->query("DELETE FROM ".$ea->table('banners')." WHERE banner_id = '$k'");
				}
