	...
	elseif($action == 'modify_education')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('education')." SET photo='".$pic."' WHERE education_id='$get_education_id'");
				echo "";
		  }
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('education')." SET image='".$pic2."' WHERE education_id='$get_education_id'");
		  }

		  $back_data = $db->query("UPDATE ".$ea->table('education')." SET title_en='".$param['title_en']."', intro_cn='".$param['intro_cn']."', title_cn='".$param['title_cn']."', intro_en='".$param['intro_en']."', description_en='".htmlspecialchars($param['description_en'])."', description_cn='".htmlspecialchars($param['description_cn'])."', dateline='".$param['add_time']."' WHERE education_id='$get_education_id'");
