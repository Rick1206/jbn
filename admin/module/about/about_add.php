...
<?php
	...
	elseif($action == 'add_about')
	{
		...
		if(!$attachment_s){
			...
			$back_data = $db->query("INSERT INTO ".$ea->table('about')." (year, photo, description_en, description_cn, description_gb, orderby) VALUES ('".$param['year']."', '".$pic."', '".htmlspecialchars($param['description_en'])."', '".htmlspecialchars($param['description_cn'])."', '".htmlspecialchars($param['description_gb'])."', '".$param['orderby']."')");
			if($back_data)
			{
