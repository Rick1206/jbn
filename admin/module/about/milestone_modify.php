...
<?php
   ...
   if(empty($action))
   {
	if($get_milestone_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('milestone'). 'WHERE milestone_id='.$get_milestone_id;
	...
	elseif($action == 'modify_milestone')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('milestone')." SET photo='".$pic."' WHERE milestone_id='$get_milestone_id'");
		  }
		  		  
		  $dateline = $param['year']."-".$param['month']."-01";
		  
		  $back_data = $db->query("UPDATE ".$ea->table('milestone')." SET dateline='".$dateline."', description_en='".htmlspecialchars($param['description_en'])."', description_cn='".htmlspecialchars($param['description_cn'])."', description_gb='".htmlspecialchars($param['description_gb'])."' WHERE milestone_id='$get_milestone_id'");
