...
<?php
   ...
   if(empty($action))
   {
	if($get_about_id != '')
	{
			...
			$sql = 'SELECT * FROM '.$ea->table('about'). 'WHERE about_id='.$get_about_id;
	...
	elseif($action == 'modify_about')
	{
	  ...
	  {	
		  ...
		  if($attachment == 1)
		  {
				...
				$db->query("UPDATE ".$ea->table('about')." SET photo='".$pic."' WHERE about_id='$get_about_id'");
		  }
		  		  
	  
		  $back_data = $db->query("UPDATE ".$ea->table('about')." SET year='".$param['year']."', description_en='".htmlspecialchars($param['description_en'])."', description_cn='".htmlspecialchars($param['description_cn'])."', description_gb='".htmlspecialchars($param['description_gb'])."', orderby='".$param['orderby']."' WHERE about_id='$get_about_id'");
