...
<?php
	...
	elseif($action == 'add_milestone')
	{
		...
		if(!$attachment_s){
			...
			$back_data = $db->query("INSERT INTO ".$ea->table('milestone')." (dateline, photo, description_en, description_cn, description_gb) VALUES ('".$dateline."', '".$pic."', '".htmlspecialchars($param['description_en'])."', '".htmlspecialchars($param['description_cn'])."', '".htmlspecialchars($param['description_gb'])."')");
			if($back_data)
			{
				//milestone_xml();
