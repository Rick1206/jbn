		...
		foreach($photos as $photo) {
			...
			while ($data = fgetcsv ($handle, 1000, ",")) {
    			if ($row>1) {
        			...
        			$db->query("INSERT INTO ".$ea->table('news')." (news_type, title_cn, description_cn, dateline) VALUES (2, '".$title."', '".$detail."', '".$data[2]."')");
				}
				$row++;
			}
			fclose ($handle);
...
?>
<form action="csv2.php" method="post" enctype="multipart/form-data" name="csv" id="csv">
